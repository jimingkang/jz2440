...
static unsigned int *mmc_cmd(unsigned short cmd, unsigned long arg, unsigned short flags)
{
static unsigned int resp[5];
S3C24X0_REG32 ccon, csta;
unsigned int csta_rdy_bit = S3C2410_SDICMDSTAT_CMDSENT;
memset(resp, 0, sizeof(resp));
debug("mmc_cmd CMD%d arg=0x%08x flags=%x\n", cmd, arg, flags);
//sdi->SDICSTA =(S3C24X0_REG32) 0xffffffff;
//sdi->SDIDSTA =(S3C24X0_REG32)0xffffffff;
//sdi->SDIFSTA = (S3C24X0_REG32)0xffffffff;
//sdi->SDICARG =(S3C24X0_REG32)arg;
...
{
unsigned int dcon, fifo;
unsigned int *dst_u32 = (u_int32_t *)dst;
unsigned int *resp;
if (len == 0)
return 0;
...
dcon |= S3C2410_SDIDCON_RXAFTERCMD|S3C2410_SDIDCON_XFER_RXSTART; if (wide)
...
debug("src %lx dst %lx end %lx pstart %lxpend %lxastart %lxaend %lx\n",
src, (ulong)dst, end, part_start, part_end, aligned_start, aligned_end);
if (part_start) {
part_len = mmc_block_size - part_start;
debug("ps src %lx dst %lx end %lx pstart %lxpend %lxastart %lxaend %lx\n", src, (ulong)dst, end, part_start, part_end, aligned_start, aligned_end);
...
debug("al src %lx dst %lx end %lx pstart %lxpend %lxastart %lxaend %lx\n", src, (ulong)dst, end, part_start, part_end, aligned_start, aligned_end);
...
src, (ulong)dst, end, part_start, part_end, aligned_start, aligned_end);
if (part_end && src < end) {
debug("pe src %lx dst %lx end %lx pstart %lxpend %lxastart %lxaend %lx\n", src, (ulong)dst, end, part_start, part_end, aligned_start, aligned_end);
...
u_int32_t *resp;
 ...
 S3C24X0_CLOCK_POWER *  const clk_power = S3C24X0_GetBase_CLOCK_POWER();//s3c24x0_get_base_clock_power();
...
/* try to get cardid */
resp = mmc_cmd(MMC_CMD_ALL_SEND_CID, 0, CMD_F_RESP|CMD_F_RESP_LONG); if (resp) {
if (!is_sd) {
...
sprintf((char *) mmc_dev.revision, "%d.%d",cid->prv >> 4, cid->prv & 15);
}
